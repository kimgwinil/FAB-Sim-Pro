...
- 블레이드 다이싱과 레이저 다이싱의 차이를 안다
- 다이싱 테이프와 웨이퍼 고정의 중요성을 이해한다
- 칩핑·크랙·버 등 대표 불량 원인을 안다
...
### 커프 폭과 수율 손실
커프 폭은 블레이드 두께와 측면 마모, 스핀들 런아웃, 웨이퍼 진동의 영향을 받습니다. 커프 폭이 넓어지면 동일 웨이퍼에서 사용할 수 있는 유효 다이 면적이 줄고, 스크라이브 라인 여유가 작은 제품에서는 패턴 손상 위험이 커집니다.

### 실습 시뮬레이터 관찰 항목
슬라이더를 조정할 때 다음 관계를 관찰합니다.
- 이송 속도 증가 → 기계적 부하 증가 → 칩핑 증가
- 냉각수 부족 → 절삭 온도 증가 → 열손상 위험 증가
- 블레이드 마모 증가 → 커프 폭 불안정 → 다이 가장자리 품질 저하
...
## 학습 목표
- 다이 본딩의 목적과 역할을 설명한다
- 리드프레임, 기판, 인터포저의 기본 개념을 안다
- 에폭시, DAF, 솔더 등 부착 재료의 차이를 안다
- 위치 정렬, 접착 두께, 경화 조건의 중요성을 이해한다
- 보이드, 틸트, 오염 등 대표 불량의 원인과 영향을 안다
...
## 학습 목표
- 칩과 외부 단자를 전기적으로 연결해야 하는 이유를 안다
- 와이어 본딩과 플립칩 본딩의 원리·흐름을 설명한다
- 범프·패드·기판·솔더 접합의 개념을 안다
- 본딩 불량의 종류와 검사 방법을 안다
...
## 대표 불량
**와이어 본딩** — 리프트(접합부 떨어짐), 와이어 스윕(몰딩 중 휨), 넥 크랙(목 부분 균열), 쇼트(와이어끼리 접촉), 미스 본드(위치 오류).
...
Au-Al 계면에는 AuAl계 금속간화합물(IMC)이 형성됩니다. 적절한 IMC는 접합 강도에 필요하지만, 고온 보관 중 확산 속도 차이로 공공이 축적되면 Kirkendall void가 발생해 접합부 리프트나 전기적 오픈으로 이어질 수 있습니다.

### 와이어 루프 역학
와이어 루프는 단순 곡선이 아니라 전기적·기계적 제약을 동시에 만족해야 합니다. 루프가 낮으면 다이 에지 쇼트 위험이 증가하고, 지나치게 높거나 길면 몰딩 유동 중 와이어 스윕이 커집니다. 고속 신호에서는 와이어 길이가 기생 인덕턴스와 임피던스에 영향을 줍니다.
...
- EMC와 패키지 보호 기능의 관계를 안다
- 트랜스퍼/컴프레션 몰딩의 차이를 안다
...
### 경화 수축과 잔류응력
EMC는 경화하면서 화학적 수축이 발생하고, 고온 성형 후 상온으로 냉각되면서 열수축이 추가됩니다. 실리콘, 리드프레임, EMC의 열팽창계수(CTE)가 다르면 계면에 전단응력이 생기며 델라미네이션이나 패키지 크랙으로 이어질 수 있습니다.
...
### 실습 시뮬레이터 관찰 항목
- 전이 압력 증가 → 충전성 향상, 플래시·와이어 스윕 위험 증가
...
- 프로브 카드·ATE 등 테스트 장비의 역할을 안다
- 전기적 검사 항목과 수율의 의미를 이해한다
- 번인·신뢰성 시험의 목적을 안다
...
## 학문적 심화
### 테스트는 측정 시스템이다
테스트 결과는 제품 특성뿐 아니라 측정 시스템의 정확도, 반복성, 접촉 안정성에도 영향을 받습니다. 좋은 칩이 소켓 접촉 불량 때문에 불량으로 판정되는 경우를 **False Fail**이라 하고, 불량 칩이 통과하는 경우를 **Test Escape**라고 합니다. 양산에서는 두 오류를 모두 줄이는 것이 중요합니다.
...
### 번인과 Arrhenius 가속
번인은 고온·전압 스트레스로 잠재 불량을 조기에 드러내는 시험입니다. Arrhenius 모델은 온도가 올라갈수록 열활성화 고장 메커니즘이 빨라진다고 설명합니다. 단, 모든 고장 메커니즘이 같은 활성화 에너지를 갖는 것은 아니므로, 실제 제품에서는 시험 조건의 타당성 검증이 필요합니다.
...
- 전압 변동 증가 → 파라메트릭 Fail 가능성 증가
- 번인 온도 증가 → 가속 계수 증가, 과도 조건은 스트레스 손상 가능
